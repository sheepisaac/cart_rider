...
def start_camera_stream(drive_status, streaming_option):
    ...
    rclpy.init()
    deblur_enabled = streaming_option != '3' and os.getenv('CART_RIDER_DEBLUR', '').strip().lower() in (
        '1',
        'true',
        'yes',
        'y',
    ...
    return camera_stream, streamer, server, spin_thread, server_thread


def stop_camera_stream(streamer, server, spin_thread, server_thread):
    import rclpy
...
def run_stream(ctrl_cart, streaming_option):
    ...
    camera_stream, streamer, server, spin_thread, server_thread = start_camera_stream(drive_status, streaming_option)
    ...
    try:
        ...
        stop_camera_stream(streamer, server, spin_thread, server_thread)
